...
if not os.path.exists(os.path.join(save_dir, 'disc_img_r18_e5_2')):
    os.makedirs(os.path.join(save_dir, 'disc_img_r18_e5_2'))
if not os.path.exists(os.path.join(save_dir, 'disc_mask_r18_e5_2')):
    os.makedirs(os.path.join(save_dir, 'disc_mask_r18_e5_2'))

# %%
import timm
from torchvision import transforms
discriminator = timm.create_model('resnet18', num_classes=2, pretrained=False)
discriminator.load_state_dict(torch.load(f'weights_2/dis_bezier224_5_0.2_0.05_1d1_r18_e5_run{run}.pth'))
...
while cnt < N_train and args.idx == 0:
    ...
    if real_prob > 0.5:
        ...
        image.save(os.path.join(save_dir, 'disc_img_r18_e5_2', f"{cnt}-{label}.png"))
        palette = [
        ...
        mask.save(os.path.join(save_dir, 'disc_mask_r18_e5_2', f"{cnt}-{label}.png"))
    else:
