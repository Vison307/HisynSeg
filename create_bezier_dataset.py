...
# %%
def get_bezier_loop_mask(n, scale, rad=0.2, edgy=0.05, radius_min=10, radius_max=15, patience=15):
    a = get_random_points(n=n, scale=scale // 3 * 2)
    x,y, _ = get_bezier_curve(a,rad=rad, edgy=edgy)
    delta_x = random.randint(0, scale // 3)
    delta_y = random.randint(0, scale // 3)
...
# %%
def synthesize_one(mask_fn, background_class="tumor", foreground_class="stroma"):
    ...
    if mask_fn == 'get_bezier_mask':
        bezier_mask = get_bezier_mask(n=12, scale=224)
    elif mask_fn == 'get_bezier_loop_mask':
        bezier_mask = get_bezier_loop_mask(n=12, scale=224)
...
# %%
def synthesize_and_save(save_dir, i, mask_fn, background_class="tumor", foreground_class="stroma"):
    synthesized_image, synthesized_mask = synthesize_one(mask_fn, background_class, foreground_class)
    ...
    synthesized_image.save(os.path.join(save_dir, 'img', f"{i:05d}-[1, 1, 0].png"))
    synthesized_mask.save(os.path.join(save_dir, 'mask', f"{i:05d}-[1, 1, 0].png"))
    return

# %%
save_dir = "data/WSSS4LUAD/bezier224_0.2_0.05_2d3"
...
N_train = 2_500
for i in tqdm(range(10_000, 10_000+N_train), total=N_train):
    synthesize_and_save(save_dir, i, 'get_bezier_mask', background_class="normal", foreground_class="stroma")

# Parallel(n_jobs=4)(delayed(synthesize_and_save)(save_dir, i, 'get_bezier_mask', background_class="tumor", foreground_class="stroma") for i in tqdm(range(N_train), total=N_train))

for i in tqdm(range(10_000+N_train, 10_000+2*N_train), total=N_train):
    synthesize_and_save(save_dir, i, 'get_bezier_mask', background_class="stroma", foreground_class="normal")
# Parallel(n_jobs=4)(delayed(synthesize_and_save)(save_dir, i, 'get_bezier_mask', background_class="stroma", foreground_class="tumor") for i in tqdm(range(N_train, 2*N_train), total=N_train))

# %%
save_dir = "data/WSSS4LUAD/bezier_loop224_0.2_0.05_2d3"
...
N_train = 2_500
for i in tqdm(range(10_000, 10_000+N_train), total=N_train):
    synthesize_and_save(save_dir, i, 'get_bezier_loop_mask', background_class="normal", foreground_class="stroma")
# Parallel(n_jobs=4)(delayed(synthesize_and_save)(save_dir, i, 'get_bezier_loop_mask', background_class="tumor", foreground_class="stroma") for i in tqdm(range(N_train), total=N_train))

for i in tqdm(range(10_000+N_train, 10_000+2*N_train), total=N_train):
    synthesize_and_save(save_dir, i, 'get_bezier_loop_mask', background_class="stroma", foreground_class="normal")
# Parallel(n_jobs=4)(delayed(synthesize_and_save)(save_dir, i, 'get_bezier_loop_mask', background_class="stroma", foreground_class="tumor") for i in tqdm(range(N_train, 2*N_train), total=N_train))
