...
parser.add_argument('--idx', type=int, default=0, help='index of the dataset, range [0, 1, 2, 3]', choices=range(4))
...
# np.random.seed(42)
# random.seed(42)
train_dir = Path("/home/fzj/tmpfs/BCSS-WSSS/training")
...
discriminator.load_state_dict(torch.load(f'./weights/dis_bcss_mosaic_2_112_r18_e3_run{run}.pth'))
...
while cnt < N:
    ...
    sys.stdout.write(f"Progressed {i}-{i+len(input_batch)}; Used time: {end_time - start_time:.2f}s; Total time: {total_time:.2f}s; Remain time: {total_time/(cnt-1800*args.idx+1)*1800 - total_time} \r")
    i += len(input_batch)
